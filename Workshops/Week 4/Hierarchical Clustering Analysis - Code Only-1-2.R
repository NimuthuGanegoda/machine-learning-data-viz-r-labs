...
install.packages(c(
  "tidyverse", "dendextend", "cluster", "factoextra", "gplots",
...
rownames(dat) <- dat[, ncol(dat)]
# Name the rows by the company names
dat <- dat[, 1:(ncol(dat) - 1)]
# Remove the "Company" variable as it's not required anymore.
View(dat)
...
fviz_dend(dhc.pu, # Clustering model
  ...
  rect = TRUE
) # Draw a rectangle to encompass each cluster
...
## ---------------------------------------------------------------------------------------
divcoef.pu <- dhc.pu$dc
divcoef.pu # Divisive coefficient
coph.dhc.pu <- cor_cophenetic(dhc.pu, dist.pu)
coph.dhc.pu # Cophenetic correlation
...
## ---------------------------------------------------------------------------------------
Group <- cutree(as.hclust(ahc.pu.ave), k = 5)
# Assign group membership to objects.
fviz_cluster(list(data = dat.z, cluster = Group), ggtheme = theme_minimal())


## ---------------------------------------------------------------------------------------
aggcoef.ave <- ahc.pu.ave$ac
aggcoef.ave # Agglomerative coefficient
coph.cor.ave <- cor_cophenetic(ahc.pu.ave, dist.pu)
coph.cor.ave # Cophenetic corrrelation
...
aggcoef.ave2 <- ahc.pu.ave2$ac
aggcoef.ave2 # Agglomerative coefficient
coph.cor.ave2 <- cor_cophenetic(ahc.pu.ave2, dist.pu.trp)
coph.cor.ave2

# Plot the dendrogram
fviz_dend(ahc.pu.ave2,
  cex = 0.5, k = 5,
...
aggcoef.site <- ahc.site$ac
aggcoef.site
coph.cor.site <- cor_cophenetic(ahc.site, dist.site)
coph.cor.site
