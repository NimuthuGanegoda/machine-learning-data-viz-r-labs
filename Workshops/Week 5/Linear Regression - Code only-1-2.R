...
cor(trainData[,3:8]) %>%
  corrplot(method="color", #escribe the magnitude of r by colours
           ...
           diag=FALSE)  #Do not show the main diagnal values
...
RMSE.rfe <- diff.rfe^2 %>% mean %>% sqrt;  #RMSE
...
RFE.pf <- c(RMSE=RMSE.rfe,
            Bias=bias.rfe,
            Corr=cor.rfe,
            PredR2=predR2.rfe); RFE.pf
